#!/usr/bin/env python3
"""
Minimal data generation: hardcoded diverse texts, no dataset dependencies.
Uses the full model on CPU for both descriptions and activations.
Targets ~150 training examples from 50 texts × 3 layers.
"""
...
# 50 diverse text passages covering different languages, topics, styles
TEXTS = [
    ...
    "日本の桜の季節は毎年多くの観光客を魅了します。公園や川沿いの美しい桜並木は春の象徴として愛されています。",
...
def main():
    ...
    parser.add_argument("--max_desc_tokens", type=int, default=96)
    ...
    print("Loading Qwen3-8B on CPU...")
    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-8B", trust_remote_code=True)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    model = AutoModelForCausalLM.from_pretrained(
        "Qwen/Qwen3-8B", dtype=torch.bfloat16, trust_remote_code=True,
    )
    model.eval()
    ...
    print(f"\nProcessing {len(TEXTS)} texts, layers {source_layers}...")
    start = time.time()
    ...
    for i, text in enumerate(TEXTS):
        ...
        try:
            ...
            descriptions_data.append({
                "text": text, "description": description, "text_idx": i,
            })
            ...
            for layer_idx in source_layers:
                ...
                torch.save(pos_acts,
                          os.path.join(args.output_dir, "activations", act_file))
            ...
            print(f"  [{i+1}/{len(TEXTS)}] {dt:.1f}s | "
                  f"examples: {total} | ETA: {eta:.0f}min | "
    ...
    elapsed = time.time() - start
    print(f"\nDone! {total} examples in {elapsed/60:.1f}min")
    print(f"Dataset: {args.output_dir}/dataset.jsonl")

    # Auto-commit
    os.system(
        "cd /home/smitop2/ao-self-distill && "
        "git add data/dataset.jsonl data/descriptions.jsonl && "
        f"git commit -m 'Generated {total} training examples from {len(TEXTS)} texts' && "
